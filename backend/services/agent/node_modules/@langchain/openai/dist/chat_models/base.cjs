...
let _langchain_core_utils_gateway = require("@langchain/core/utils/gateway");
...
var BaseChatOpenAI = class extends _langchain_core_language_models_chat_models.BaseChatModel {
	...
	_getInvocationParamsForTracing(options) {
		...
		return {
			...
			tools: params.tools.map((tool) => {
				...
				if ("headers" in redactedTool) redactedTool.headers = MCP_CREDENTIALS_REDACTED;
				if ("authorization" in redactedTool) redactedTool.authorization = MCP_CREDENTIALS_REDACTED;
				return redactedTool;
			})
		};
	}
	_getCallOptionsForTracing(options, _invocationParams) {
		...
		return {
			...
			tools: options.tools.map((tool) => {
				if (typeof tool !== "object" || tool === null || !("type" in tool) || tool.type !== "mcp") return tool;
				const redactedTool = { ...tool };
				if ("headers" in redactedTool) redactedTool.headers = MCP_CREDENTIALS_REDACTED;
				if ("authorization" in redactedTool) redactedTool.authorization = MCP_CREDENTIALS_REDACTED;
				return redactedTool;
	...
	*/
	...
	constructor(fields) {
		...
		const gatewayConfig = (0, _langchain_core_utils_gateway.resolveLangSmithGatewayConfig)({
			baseURL: fields?.configuration?.baseURL ?? ((0, _langchain_core_utils_env.getEnvironmentVariable)("OPENAI_API_BASE") || (0, _langchain_core_utils_env.getEnvironmentVariable)("OPENAI_BASE_URL") || void 0),
			providerPath: "openai/v1"
		});
		this.apiKey = fields?.apiKey ?? configApiKey ?? gatewayConfig.apiKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("OPENAI_API_KEY");
		...
		this._addVersion("@langchain/openai", "1.5.8");
	}
	/**
