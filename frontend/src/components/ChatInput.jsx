...
const handleSendMessage = async () => {
  ...
  if(conversation.title=="New Chat"){
    const conv=await updateCoversation({id:conversation?._id,title:value.trim()})
  }
   ...
   placeholder:text-slate-600 leading-relaxed [scrollbar-width:none] [&::-webkit-scrollbar]:hidden
    disabled:opacity-50"
        ...
        <div className="flex items-center justify-between">
          ...
          className={`flex items-center justify-center w-8 h-8 rounded-lg border-none cursor-pointer transition-all duration-150 ${value.trim()?
          "bg-linear-to-br from-indigo-500 to-violet-700 hover:opacity-90 text-white" : "bg-white/[0.05] text-slate-600 cursor-not-allowed"}
          `}>
            <Send size={16} />
          </button>
        </div>
